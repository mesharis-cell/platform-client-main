    ...
    return (
        <div className="border border-border rounded-lg p-4">
            <div className="flex items-start justify-between mb-3">
                <div className="flex-1">
                    ...
                    <p className="text-sm text-muted-foreground">
                        Volume: {item.volumePerUnit} m³ per unit × {item.quantity} ={" "}
                        {(item.volumePerUnit * item.quantity).toFixed(1)} m³
                    </p>
                </div>
            ...
            {item.isReskinRequest && (
                <RebrandItemBadge
                    targetBrandName={
                        targetBrandName || item.reskinTargetBrandCustom || "Unknown Brand"
                    }
